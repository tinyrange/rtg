...
These files are intentionally placed under `tests/go/compiler_bugs_repros/` so existing top-level test runners that scan `tests/go/*.go` do not trigger them automatically.
