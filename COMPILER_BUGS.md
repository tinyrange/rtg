...
Compiler bugs and limitations discovered while implementing stdlib extensions (`errors`, `strconv`, `bytes`, `bufio`, `flag`, `log`) on 2026-02-27, plus later selfhost, backend, CI, and audit findings.
...
### Watch: #1 ICE in `compileGlobalInits` for package-scope initializers
...
### Watch: #7 Package-level `log` state runtime crash paths
