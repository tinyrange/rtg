...
mkdir -p "$OUT_DIR"
rm -f "$OUT_DIR"/T??.exe "$OUT_DIR"/O??.txt "$OUT_DIR"/E??.txt
rm -f "$OUT_DIR"/O??_*.txt "$OUT_DIR"/E??_*.txt "$OUT_DIR"/PROG.TIR "$OUT_DIR"/CHILD.EXE
...
tiny_parser_com="$OUT_DIR/TINY_PARSER.exe"
tiny_backend_com="$OUT_DIR/TINY_BACKEND.exe"
...
total_count=$((total_count + 1))
tiny_ok=1
if [[ "$tiny_parser_size" -gt 131072 || "$tiny_backend_size" -gt 131072 ]]; then
  tiny_ok=0
fi

rm -f "$OUT_DIR/PROG.TIR" "$OUT_DIR/CHILD.EXE"
...
if [[ "$tiny_ok" == "1" ]]; then
  ...
  # Treat CHILD.EXE presence as the success signal for this stage.
  (cd "$OUT_DIR" && go run "$ROOT_DIR/tools/comemu" "$(basename "$tiny_backend_com")" >"$tiny_backend_out" 2>"$tiny_backend_err") || true
fi
if [[ "$tiny_ok" == "1" ]]; then
  if [[ ! -f "$OUT_DIR/CHILD.EXE" ]]; then
    tiny_ok=0
  fi
fi
if [[ "$tiny_ok" == "1" ]]; then
  child_size="$(wc -c <"$OUT_DIR/CHILD.EXE" | tr -d ' ')"
  if [[ "$child_size" -gt 131072 ]]; then
    tiny_ok=0
  fi
fi
if [[ "$tiny_ok" == "1" ]]; then
  if ! (cd "$OUT_DIR" && go run "$ROOT_DIR/tools/comemu" CHILD.EXE >"$tiny_child_out" 2>"$tiny_child_err"); then
    tiny_ok=0
  fi
fi
...
if [[ "$tiny_ok" == "1" ]]; then
  pass_count=$((pass_count + 1))
  echo "$(printf "%2d" "$tiny_idx"). tiny parser+backend pipeline -> PASS (parser_bytes=$tiny_parser_size backend_bytes=$tiny_backend_size child_bytes=$(wc -c <"$OUT_DIR/CHILD.EXE" | tr -d ' '))"
else
