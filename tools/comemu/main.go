...
func main() {
	...
	maxSteps := flag.Int("max-steps", 2000000, "instruction step limit")
	flag.Parse()
	if flag.NArg() < 1 {
	...
	if len(bin) > 65536-0x100 {
		fmt.Fprintf(os.Stderr, "COM too large: %d bytes\n", len(bin))
		os.Exit(1)
	}

	warnIfLooksLike32Bit(bin)

	c := &cpu{trace: *trace, maxSteps: *maxSteps, dbgWrite: os.Getenv("COMEMU_DEBUG_WRITES") == "1"}
	if err := c.loadCOM(defaultPSP, bin, progArgs); err != nil {
		fmt.Fprintf(os.Stderr, "load COM: %v\n", err)
		os.Exit(1)
	}
...
func (c *cpu) step() error {
	...
	default:
		return fmt.Errorf("unsupported opcode %02x at %04x:%04x (bytes %02x %02x %02x %02x)",
			op, c.cs, c.ip, c.mem[csip], c.mem[csip+1], c.mem[csip+2], c.mem[csip+3])
	}
}
...
func (c *cpu) exec0FAddr32(pc uint32) error {
	...
	case op2 == 0xb6 || op2 == 0xb7:
		...
		if op2 == 0xb6 {
			c.setReg16(reg, uint16(c.mem[addr]))
		} else {
...
func (c *cpu) ea32(mod, rm byte, dispStart uint32) (uint32, int, error) {
	...
	switch mod {
	case 1:
		disp = int32(int8(c.mem[dispStart+uint32(consumed)]))
		consumed++
	case 2:
...
func (c *cpu) handleInt(vec byte) error {
	...
	case 0x21:
		...
		case 0x40:
			...
			if int(ptr)+int(count) > len(c.mem) {
				c.cf = true
				c.ax = 0x0008
				return nil
			}
			data := c.mem[ptr : ptr+uint32(count)]
			if c.trace {
...
func (c *cpu) exec83(pc uint32) error {
	...
	imm := uint16(int16(int8(c.mem[pc+2])))
	v := c.reg16(rm)
	switch op {
	case 0: // add
		res := v + imm
