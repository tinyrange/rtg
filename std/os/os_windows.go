...
func Getenv(key string) string {
	...
	envPtr, _, _ := runtime.Syscall(SYS_GET_ENV_STRINGS, 0, 0, 0, 0, 0, 0)
	if envPtr == 0 {
		return ""
	}
...
func ListDir(dirname string) ([]string, error) {
	...
	handle, _, errn := runtime.Syscall(SYS_FIND_FIRST_FILE, runtime.Sliceptr(pbuf), runtime.Sliceptr(findData), 0, 0, 0, 0)
	if errn != 0 {
		return nil, Errno(errn)
	}
	...
	for {
		...
		_, _, errn = runtime.Syscall(SYS_FIND_NEXT_FILE, handle, runtime.Sliceptr(findData), 0, 0, 0, 0)
		if errn != 0 {
			break
		}
	}

	runtime.Syscall(SYS_FIND_CLOSE, handle, 0, 0, 0, 0, 0)
	return names, nil
}
...
func init() {
	...
	cmdLinePtr, _, _ := runtime.Syscall(SYS_GET_CMDLINE, 0, 0, 0, 0, 0, 0)
	if cmdLinePtr == 0 {
		return
	}
